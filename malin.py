...
from InimigoMapa import InimigoMapa
...
inimigos_mapa_group = pygame.sprite.Group()
try:
    for obj in tmx_data.get_layer_by_name("Inimigos"):
        patrol_range = obj.properties.get('patrol_range', 50)
        inimigo_obj = InimigoMapa(obj.x, obj.y, patrol_range)
        inimigos_mapa_group.add(inimigo_obj)
except ValueError:
    print("Aviso: Camada 'Inimigos' não encontrada no mapa.")

# --- Variáveis de outros modos ---
jogador = None
inimigo = None
batalha = None
botoes_ataque = {}
...
while rodando:
            ...
            if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1:
                ...
                for nome_classe, botao_rect in botoes_classe.items():
                    if botao_rect.collidepoint(pos_mouse):
                        ...
                        player_exploracao.rect.center = (LARGURA_TELA / 2, ALTURA_TELA / 2)
    ...
    if game_state == "exploracao":
        ...
        inimigos_mapa_group.update(paredes)

        for inimigo_mapa in inimigos_mapa_group:
            if player_exploracao.rect.colliderect(inimigo_mapa.detection_rect):
                print("BATALHA INICIADA!")
                inimigo_mapa.kill()

                atributos_jogador = Personagem.criacao_perso_teste("Player", jogador.classe)
                jogador = Personagem(*atributos_jogador)
                ...
                try:
                    ...
                    print("Aviso: 'battle_music.mp3' não encontrado.")
                break
